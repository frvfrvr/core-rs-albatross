                ...
                TendermintReturn::Result(block) => {
                    ...
                    if result == Some(PushResult::Extended) || result == Some(PushResult::Rebranched) {
                        if block_copy.is_election_block() {
                            info!("Validator {}: Publishing Election MacroBlock #{}", id, &block_copy.header.block_number);
                        } else {
                            info!("Validator {}: Publishing Checkpoint MacroBlock #{}", id, &block_copy.header.block_number);
                        }
